...
def TrackImages():
    ...
    # Check if the treeview has children before trying to clear them
    if tv.get_children():
    ...
    exists3 = os.path.isfile(r"C:\Users\nevin\PycharmProjects\ai face\TrainingImageLabel")
    if exists3:
    ...
    with open("Attendance\Attendance_" + date + ".csv", 'r') as csvFile1:
        ...
        for lines in reader1:
            ...
            if (i > 1):
                if (i % 2 != 0):
                    id = str(lines[0]) + '   '
                    tv.insert('', 0, text=id, values=(str(lines[2]), str(lines[4]), str(lines[6])))
    csvFile1.close()
    cam.release()
    cv2.destroyAllWindows()
